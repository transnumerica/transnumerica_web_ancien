 ...
 */
	...
	public function startup(Controller $controller) {
		...
		if(!empty($this->Auth->authenticate['Form']['userModel'])) $this->settings['userModel'] = $this->Auth->authenticate['Form']['userModel'];



		if ($this->settings['autoLogin'] == true AND $this->cookieIsSet() && !$this->Auth->user()) {
		...
		if ($this->cookieIsSet()) {
			...
			foreach ($fields as $field) {
				...
				if (!empty($cookie[$field])) {
					...
					if (in_array($field, array('id'))) $this->Auth->authenticate['Form']['fields']['username'] = $field;
			...
			if(empty($this->controller->Components->Auth->request)){

				$this->controller->Components->Auth->initialize($this->controller);

				$this->controller->Components->Auth->request = $this->request;
 ...
 */
	...
	public function setCookie($data = array()) {
		...
		if (!in_array($userModel, array_keys($data))) {

			$data[$userModel] = $this->Auth->user();
		...
		if (!empty($this->request->data[$userModel])) {

			$data[$userModel] = Hash::merge($this->request->data[$userModel], $data[$userModel]);
 ...
 */
	...
	public function configureCookie($options = array()) {
		...
		$validProperties = array('domain', 'key', 'name', 'path', 'secure', 'time', 'httpOnly','type');
		...
		foreach ($options as $key => $value) {
			...
			if (in_array($key, $validProperties)) {
				...
				throw new InvalidArgumentException(__d('compte', 'Options invalid %s', $key));
