...
function myFetchTicketShow(idTicket, goHomeAfter = false){
    myFetch({url: `${window.location.origin}/m/ticket/`+idTicket, method: 'GET',
            headers:{
                'Accept': 'application/json',
                //'Content-Type': 'application/json'
            },
            thenFct: (res)=>{
                ...
                if(res?.success){
                    if(goHomeAfter) goHomeAfterFacture();
                    modalFacturation({dataWeb});
                }else{
                    if(goHomeAfter) goHomeAfterAlert();
                ...
                if(new Date().getTime() - timeOfintervalPaymentChecker >60000){
                    ...
                    myFetch({url: `${window.location.origin}/m/ticket/${res?.data?.sale_id??"-1"}`, thenFct: (resTicket)=>{
                            ...
                            if([0,1].includes(statusTicket));
                                ...
                                let dataWeb = resTicket?.data;//?.SaleDB;
                                modalFacturation({dataWeb});
                                hideChargement();
                            }else{
                                clearInterval(intervalPaymentChecker);
...
function modalFacturation({dataWeb}){
    console.log({modalFacturation: dataWeb});
    if(dataWeb?.Sale?.status ==2 ){
        ...
        let facture = {
            ...
            details: {
                ...
                Prenom: dataWeb?.User?.Info?.firstname,
                Nom: dataWeb?.User?.Info?.name,
                Telephone: dataWeb?.User?.Info?.phone,
                ...
                statut: dataWeb?.Sale?.consomme==1?'Consommé':'Réservé',//(dataWeb?.Sale?.consomme)?'Consommé':'Réservé',
...
if(typeof num_ticket_to_read !== 'undefined'){
    if(!isNaN(parseInt(num_ticket_to_read))){
        ...
        myFetchTicketShow(num_ticket_to_read, true);
...
(function ($) {
    ...
    */
    ...
    setTimeout(()=> alert(`screen w : ${window.innerWidth} - screen h : ${window.innerHeight}`),
        ...
        */
    let townSource = document.querySelector('.select-town.town-source');
    let townDest = document.querySelector('.select-town.town-destination');
    let countryDest = document.querySelector('.select-country.country-destination');

    function nettoyerSelect(leSelect){
        if(leSelect != null){
            let loption = leSelect.options[leSelect.selectedIndex];
            if((loption?.style?.display??"none") == "none"){
                leSelect.selectedIndex = -1;
            }
        }
    }

    function selectOnChangeCasNull({selectSource, selectDest}){
        let optionSelected = selectSource.options[selectSource.selectedIndex];

        if(optionSelected==null || selectSource.selectedIndex <0 || optionSelected?.style?.display=="none"){
            
            for(let opt of selectDest.options){
    ...
    townSource?.addEventListener('change', (event)=>{
        console.log({change: event?.target?.className});
        
        if(selectOnChangeCasNull({selectSource: townSource, selectDest: countryDest}));
        else{
            var firstChangeCountry = (optionValue)=>{
                countryDest.value=optionValue;
                //console.log(`${optionValue} : ${townSelect.value}`);
                firstChangeCountry = (optionValue)=>{}
            }

            
            let townSourceId = parseInt(townSource.value);
            
            let verifTownsJson = (option)=>{
                return KTownsJson[townSourceId].includes(option.getAttribute('value'));
            }
            ...
            for(const optionCountry of countryDest.children){

                let countryContainOneTown = false;
                let countryId = optionCountry.value;
                for(const option of townDest){
                    if(option.getAttribute('country_id')==countryId &&
                        verifTownsJson(option)){
                        

                        firstChangeCountry(optionCountry.value);
                        countryContainOneTown = true;
                        optionCountry.style.display = "initial";
                        break;
                        
                    }
                }

                if(!countryContainOneTown) optionCountry.style.display = "none";
        ...
        nettoyerSelect(townSource);
        nettoyerSelect(countryDest);

        const newEvent = new Event('change');
        setTimeout(()=>countryDest.dispatchEvent(newEvent), 100);
        

        
        console.log({countryDest});
    ...
    document.querySelectorAll('.select-country').forEach((countrySelect, key)=>{
        countrySelect.addEventListener('change', (event)=>{
            ...
            if(selectOnChangeCasNull({selectSource: countrySelect, selectDest: townSelect}));
            else{
                ...
                let verifTownsJson = countrySelect.classList.contains("country-source")?
                    (option)=>{
                        return KTownsJson[parseInt(option.getAttribute('value'))]!=null;
                    }:(option)=>{
                        return KTownsJson[townSourceId].includes(option.getAttribute('value'));
                    }
                ...
                for(const option of townSelect.children){
                    ...
                    testI.push(parseInt(option.getAttribute('value')));
            ...
            nettoyerSelect(countrySelect);
            nettoyerSelect(townSelect);

            const newEvent = new Event('change');
            setTimeout(()=>townSelect.dispatchEvent(newEvent), 100);
            console.log({KTownsJson, testI});

        });

        const newEvent = new Event('change');
        if(countrySelect.classList.contains("country-source")){
