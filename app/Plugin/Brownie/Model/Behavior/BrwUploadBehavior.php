...
class BrwUploadBehavior extends ModelBehavior {
	...
	public function afterSave(Model $Model, $created, $options = array()) {
		if (!empty($Model->data[$Model->alias]['file'])) {
			...
			$uploadsPath = classRegistry::init($data['ref'])->brwConfig[$uploadType][$data['category_code']]['path'];
			$dest_dir = $uploadsPath;
			if (!$created) {
				$this->_deleteFiles($uploadsPath, $data['ref'], $data['ref_id'], $Model->data['name_prev']);
			}
			if (!is_dir($dest_dir)) {
	...
	public function _copy($Model, $source, $dest) {
		...
		while (is_file($newDest)) {
			...
			$i++;
		//	if ($i == 4){ break;};
			$newDest = join(DS, $parts2) . DS . $file;
		}
	...
	public function beforeDelete(Model $Model, $cascade = true) {
		...
		$uploadType = ($Model->alias == 'Media') ? 'images' : 'files';
		$relModel = ClassRegistry::init($upload['ref']);
		$uploadsPath = $relModel->brwConfig[$uploadType][$upload['category_code']]['path'];
		$this->_deleteFiles($uploadsPath, $upload['ref'], $upload['ref_id'], $upload['name']);
		return true;
	}
