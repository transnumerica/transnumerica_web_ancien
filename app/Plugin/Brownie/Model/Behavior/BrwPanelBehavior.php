...
class BrwPanelBehavior extends ModelBehavior {
	...
	public $brwConfigDefault = array(
		...
		'fields' => array(
			'no_add' => array('level','media_id'),
			'no_edit' => array('level'),
			'no_view' => array('media_id','lien', 'level'),
			'no_fieldList' => array(),
			'hide' => array('lft', 'rght'),
			'export' => array(),
		...
		'parent' => null,
		'show_children' => true,
		'hide_children' => array('Media', 'BrwFile','children'),
		'hide_related' => array(
			'hasMany' => array('Media', 'BrwFile'),
	...
	public function afterFind(Model $Model, $results, $primary = false) {
		...
		if (!in_array($Model->alias, array('BrwFile', 'Media'))) {
			...
			if (!in_array($related['className'], array('BrwImage', 'BrwFile', 'I18nModel'))) {
				if (!$related['dependent']) {
					...
					if ($rel) {
						...
						if (isset($schema['null'])) {
							$toNullModels[] = array('model' => $rel, 'foreignKey' => $related['foreignKey']);
						} else {

							if (isset($related['foreignKey']) AND $related['foreignKey'] !== false) {
								$conditionsDelete[$rel->alias . '.' . $related['foreignKey']] = $Model->id;
							}

							$conditionsDelete[] = json_decode(str_replace('{$__cakeID__$}', $Model->id, json_encode($related['conditions'])), true);

							$hasAny = $rel->find('first', array(
								'conditions' => $conditionsDelete,
								'fields' => array('id'),
							));
							if ($hasAny) {
								return false;
	...
	public function brwConfigInit($Model) {
		...
		if (class_exists('AuthComponent')) {
			if (in_array($Model->name, array('Media', 'BrwFile', 'Content'))) {
	...
	public function _paginateConfig($Model) {
		if (empty($Model->brwConfig['paginate']['fields'])) {
			$listableTypes = array(
				'integer', 'biginteger', 'float', 'string', 'boolean',
				'date', 'datetime', 'time', 'timestamp',
			);
			$fields = array(); $i = 0; $schema = (array)$Model->schema();
			$blacklist = array_merge(
				array('lft', 'rght', 'parent_id', 'created', 'modified', 'updated'),
				$Model->brwConfig['fields']['hide'],
				$Model->brwConfig['fields']['no_view']
			);
			foreach ($schema as $key => $values) {
				if ((in_array($values['type'], $listableTypes) OR stripos($values['type'], 'enum') === 0) and !in_array($key, $blacklist)) {
					$key2 = $key;
					if (!empty(Configure::read('Config.languages'))) {
						foreach (Configure::read('Config.languages') as $lang) {
							if (strpos($key, '_'.$lang) AND !strpos($key, '_'.Configure::read('Config.language'))) unset($key2);
						}
					}
	...
	public function _uploadsConfig($Model) {
		foreach (array('BrwFile' => 'files', 'Media' => 'images') as $uploadModel => $uploadType) {
			if ($Model->brwConfig[$uploadType]) {
				$Model->bindModel(array('hasMany' => array($uploadModel => array(
					'foreignKey' => 'ref_id',
					'conditions' => array($uploadModel . '.ref' => $Model->name)
				))), false);
				...
				foreach ($Model->brwConfig[$uploadType] as $key => $value) {
					...
					$Model->brwConfig[$uploadType][$key]['path'] = str_replace('%y', date("Y"), $Model->brwConfig[$uploadType][$key]['path']);
	...
	public function _addImagePaths($r, $Model) {
		foreach ($r as $key => $value) {
			if ($key === 'Media') {
				$thisModel = $Model;
				if(!empty($r[$key][0]['ref']) and $r[$key][0]['ref'] != $thisModel->alias) {
					$thisModel = ClassRegistry::getObject($r[$key][0]['ref']);
				}
				$r[$key] = $this->_addMediaPaths($value, $thisModel);
			} else {
				if(is_array($value)) {
	...
	public function _addMediaPaths($r, $Model) {
	...
	public function _addBrwUploadsPaths($r, $Model, $fileType) {
		...
		foreach ($r as $key => $value) {
			...
			$realPathFile = $Model->brwConfig[$fileType][$value['category_code']]['realpath']
				. DS . $Model->brwConfig[$fileType][$value['category_code']]['path'] . DS . $value['name'];
			...
			if ($isPublic) {
				...
				$url = Router::url(array(
					...
					$Model->alias, $value['ref_id'], $value['category_code'], $value['name']
				));
			}
			$paths = array(
				'public' => $isPublic,
	...
	public function _attachUploads($Model) {
		if (!empty($Model->brwConfig['images'])) {
			$Model->bindModel(array('hasMany' => array('Media' => array(
				'foreignKey' => 'ref_id',
				'conditions' => array('Media.ref' => $Model->alias),
				'dependent' => true,
			))), false);
			/*
			commented because of an error in testing, revisar despues
			$Model->Media->bindModel(array('belongsTo' => array($Model->name => array(
				'foreignKey' => 'ref_id',
				'conditions' => array('Media.ref' => $Model->name),
				'dependent' => true,
			))), false);*/
		}
		if (!empty($Model->brwConfig['files'])) {
			$Model->bindModel(array('hasMany' => array('BrwFile' => array(
				'foreignKey' => 'ref_id',
				'conditions' => array('BrwFile.ref' => $Model->alias),
				'dependent' => true,
			))), false);
			/*
			commented because of an error in testing, revisar despues
				...
				foreach (array('minYear', 'maxYear') as $yearType) {
					if (empty($Model->brwConfig['fields']['date_ranges'][$field][$yearType])) {
						$Model->brwConfig['fields']['date_ranges'][$field][$yearType] =
							date('Y') + (($yearType == 'maxYear')? 20: -120);
					} else {
					...
					if (!isset($Model->brwConfig['fields']['date_ranges'][$field]['timeFormat'])) {
						$Model->brwConfig['fields']['date_ranges'][$field]['timeFormat'] = Configure::read('brwSettings.timeFormat');
					}
				}
			}
			if (in_array($config['type'], array('time'))) {
				$Model->brwConfig['fields']['date_ranges'][$field]['timeFormat'] = Configure::read('brwSettings.timeFormat');
			}
		}
	}
	...
	public function brwSchema($Model) {
        ...
        if (!empty($retSchema)) {
            ...
            $source = explode('/', $dataSource->config['datasource'])[1];

            if ($source == 'Mysql') {
                    $cols = $Model->query('SHOW FULL COLUMNS FROM ' . $Model->table);
                    foreach ($cols as $col) {
                        $col = $col['COLUMNS'];
                        $retSchema[$col['Field']]['columns'] = explode('(', explode(' ', $col['Type'])[0])[0];
                    }
            }
