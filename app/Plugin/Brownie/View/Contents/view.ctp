<div class="view" id="<?php echo $model;?>_view">
		...
		<ul class="actions neighbors">
			...
			if (!empty($neighbors['prev'])) {
				...
				<li class="prev">
					...
					array('action' => 'view', $model, $neighbors['prev'][$model][$primaryKey]) + $this->params['named'],
			...
			if (!empty($neighbors['next'])) {
				...
				<li class="next">
					...
					array('action' => 'view', $model, $neighbors['next'][$model][$primaryKey]) + $this->params['named'],
	...
	foreach ($record[$model] as $key => $value) {
		if (strstr($key, '_lang')) {
			$keyO = str_replace('_lang','', $key);
			if (!empty($record[$model][$key]) AND in_array($keyO, array_keys($record[$model]))) {
				unset($record[$model][$keyO]);
			}else{
				unset($record[$model][$key]);
				$record[$model][$keyO] = sprintf(__d('brownie', '%s <i>(Pas encore traduit)</i>'), $record[$model][$keyO]);
			}
		}	
	}
	...
	foreach ($record[$model] as $field_name => $field_value) {
		if (!empty($schema[$field_name]) and !in_array($field_name, $brwConfig['fields']['no_view'])) {
			echo '
			<tr>
				<td class="label">' . __(str_replace(' Lang','', $brwConfig['fields']['names'][$field_name])) . '</td>';
				if (in_array($field_name, $i18nFields)) {
					echo '
					<td class="multiLang">
					' . $this->element('i18n_view_field', array('data' => $record['BrwI18n_' . $field_name])) . '
					</td>';
				}elseif (strstr($field_name, '_lang')) {
					echo '
					<td class="multiLang">
					' . $this->element('i18n_view_trans', array('data' => $field_value)) . '
					</td>';
				}else {
					echo '<td class="fcktxt">';
					
					if (!is_array($field_value)) {

						if (pathinfo($field_value, PATHINFO_EXTENSION)) {
							echo $this->Html->link(pathinfo($field_value)['basename'],$field_value);
						}else{							
							echo (($field_value === null or $field_value === '') ? '&nbsp;' : $field_value );
						}

					}else{

						$this->Brownie->arrayview($field_value);
...
<?php
$uploadModels = array('images' => 'Media', 'files' => 'BrwFile');
