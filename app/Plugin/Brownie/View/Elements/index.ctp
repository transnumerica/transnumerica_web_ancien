...
<div class="index clearfix">
	...
	<div class="actions">
		...
		$aSuerystring = array();
		if($model != $this->request->pass[0]){
			$aSuerystring = array('?' => array('parentmodel' => $this->request->pass[0]));
		}
		...
		if ($permissions[$model]['add']) {
			...
			<li class="add">' . $this->Html->link(
				...
				array_merge(array('action' => 'edit', $model), $filters, $aSuerystring)
			) . '</li>';
		}
...
if ($records) {
	...
	foreach ($records as $record):
		...
		$class = ($permissions[$model]['delete']) ? 'row_delete_multiple' : '';
		...
		if ($permissions[$model]['delete']) {
			...
			echo '
			<td class="delete_multiple" width="10px">
				'.$this->Form->input('Content.'.$record[$model]['primaryKey'].'.', array('type' => 'checkbox', 'div' => false, 'hiddenField' => false, 'label' => false, 'id' => false, 'value' => $record[$model][$record[$model]['primaryKey']])).'
			</td>';
		}
		foreach($brwConfig['paginate']['fields'] as $field_name) {
			if (!empty($schema[$field_name])) {
				echo '<td class="' . $field_name . ' ' . $schema[$field_name]['class'] . ' field">';

				if (pathinfo($record[$model][$field_name], PATHINFO_EXTENSION)){
					echo $this->Html->link(pathinfo($record[$model][$field_name])['basename'],$record[$model][$field_name]);
				}else{
					echo (($record[$model][$field_name] === null or $record[$model][$field_name] === '') ? '&nbsp;' : $record[$model][$field_name]);
				}
				echo '</td>';
				if ($field_name == 'id') {
					foreach ($brwConfig['paginate']['images'] as $indexImageKey) {
