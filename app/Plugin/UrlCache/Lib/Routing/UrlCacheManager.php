...
class UrlCacheManager {
	...
	public static function init(View $View) {
		...
		if (Configure::read('UrlCache.pageFiles')) {
			...
			if ($cache = Cache::read(self::$cachePageKey, 'url')) {
				self::$cachePage = unserialize(gzinflate($cache));
			}
			if (Configure::read('debug')) {
				Configure::write('UrlCacheDebug.countPage', count(self::$cachePage));
			}
		}
		if ($cache = Cache::read(self::$cacheKey, 'url')) {
			self::$cache = unserialize(gzinflate($cache));
		}
		if (Configure::read('debug')) {
	...
	public static function finalize() {
		...
		Cache::write(self::$cacheKey, gzdeflate(serialize(self::$cache)), 'url');
		if (Configure::read('UrlCache.pageFiles') && !empty(self::$cachePage)) {
			Cache::write(self::$cachePageKey, gzdeflate(serialize(self::$cachePage)), 'url');
		}
	}
