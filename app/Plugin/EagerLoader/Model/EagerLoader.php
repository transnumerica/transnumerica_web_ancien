 ...
 */
	private function attachAssociations(Model $model, $path, array $query) { // @codingStandardsIgnoreLine
		...
		$joins = $notJoinsNeccesaries = array();
		...
		if ($joins) {
			...
			if (!empty($query['joins'])) {
				$notJoinsNeccesaries = Hash::extract($query['joins'], '{n}.alias');
			}

			foreach ($joins['joins'] as $key_join => $join) {
				if (in_array($join['alias'], $notJoinsNeccesaries)) {
					unset($joins['joins'][$key_join]);
				}
			}

			$query = array_merge_recursive($query, $joins);
		}
	...
	private function mergeExternalExternal(Model $model, array $results, array $meta) { // @codingStandardsIgnoreLine
		...
		if (!empty($finderQuery)) {
			...
			if ($targetKey !== false) {
				...
				if (!function_exists('cakeIdReplaceQuery')) {
					function cakeIdReplaceQuery($values, $replaceFind, $replaceData) {
						foreach ($values as $key => $value) {
					        if (is_array($value)) {
					            $value = cakeIdReplaceQuery($value, $replaceFind, $replaceData);
					            $values[$key] = $value;
					        }else{
					        	if($values[$key] == $replaceFind){
						            $values[$key] = $replaceData;
					        	}
					        }
					    }
					    return $values;
				...
				$options['conditions'] = cakeIdReplaceQuery($options['conditions'], '{$__cakeID__$}', $ids);
			}
		...
		foreach ($results as &$result) {
			...
			$assoc = array();
			foreach ($assocResults as $assocResult) {
				if ($targetKey === false OR (string)$result[$parentAlias][$parentKey] === (string)$assocResult['EagerLoaderModel']['assoc_id']) {
		...
		foreach ($contain as $key => $val) {
			...
			if (!isset($this->containOptions[$key])) {
				if (strpos($key, '.') !== false) {
					...
					if ($expanded AND is_array($expanded)) {
						list($key, $val) = array(array_keys($expanded)[0], array_values($expanded)[0]);
					}else{
