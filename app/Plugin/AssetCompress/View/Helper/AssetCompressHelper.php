 ...
 */
	public function addScript($files, $target = ':hash-default.js') {
		...
		$this->_runtime['js'][$target] = $this->view->getCurrentType();
		$runtime_layout = $runtime = array();
		foreach ($this->_runtime['js'] as $key => $type) {
			if ($type == 'layout') $runtime_layout[$key] = $type;
			else$runtime[$key] = $type;
		}
		$this->_runtime['js'] = array_merge($runtime_layout, $runtime);
 ...
 */
	public function addCss($files, $target = ':hash-default.css') {
		...
		$this->_runtime['css'][$target] = $this->view->getCurrentType();
		$runtime_layout = $runtime = array();
		foreach ($this->_runtime['css'] as $key => $type) {
			if ($type == 'layout') $runtime_layout[$key] = $type;
			else$runtime[$key] = $type;
		}
		$this->_runtime['css'] = array_merge($runtime_layout, $runtime);
